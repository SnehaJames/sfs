...
class Admin extends CI_Controller
{
    ...
    function student_bulk_import()
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');

        $page_data['page_name']  = 'student_bulk_import';
        $page_data['page_title'] = get_phrase('student_bulk_import');
        $this->load->view('backend/index', $page_data);
    }

    function student_bulk_import_api()
    {
        $date = new DateTime();

        $fileName = 'uploads/' . $date->getTimestamp() . '.xlsx';

        move_uploaded_file($_FILES['userfile']['tmp_name'], $fileName);
        $filename = $_FILES['userfile']['name'];

        $cfile = new CURLFile(realpath($fileName),'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',$date->getTimestamp() . '.xlsx');
        ...
        $ch = curl_init($this->config->item('smsapi').'FileUpload/?classId='.$classid.'&sectionId='.$sectionid);
        curl_setopt($ch, CURLOPT_POST, true);
        //curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        //curl_setopt($ch, CURLOPT_TIMEOUT_MS, 5);
        //curl_setopt($ch, CURLOPT_NOSIGNAL, 5);
        curl_setopt($ch, CURLOPT_POSTFIELDS, array("filedata" => $cfile));
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('Data will be imported shortly.'));
        redirect(base_url() . 'index.php?admin/student_bulk_import/', 'refresh');
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $randomPassword = substr( md5( rand(100000000,20000000000) ) , 0,7);
            ...
            {
                $data['password']       = sha1($randomPassword);
            }
            ...
            if($data['email']  != null && $data['email'] != '')
            {
                $this->email_model->account_opening_email('student', $data['email'],$randomPassword); //SEND EMAIL ACCOUNT OPENING EMAIL
            }
    ...
    function parent($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $query = $this->db->get_where('parent', array(//making selection
                'email' => $data['email']
            ));

            if ($query->num_rows() != 0) {
                $this->session->set_flashdata('flash_message' , get_phrase('Email already in use.'));
            ...
            if($data['email']  != null && $data['email'] != '')
            {
                $this->email_model->account_opening_email('parent', $data['email'],$randomPassword, $data['bot_auth_key']); //SEND EMAIL ACCOUNT OPENING EMAIL
            }
    ...
    function teacher($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $query = $this->db->get_where('teacher', array(//making selection
                'email' => $data['email']
            ));
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            $this->email_model->account_opening_email('teacher', $data['email'], $randomPassword); //SEND EMAIL ACCOUNT OPENING EMAIL
            redirect(base_url() . 'index.php?admin/teacher/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']        = $this->input->post('name');
    ...
    function  bot_profile($param1 = '')
    {
        ...
        if ($param1 == 'do_update') {
            ...
            $data['description'] = $this->input->post('bot_name');
            $this->db->where('type', 'bot_name');
            $this->db->update('bot_profile', $data);

            $data['description'] = $this->input->post('about_school');
            $this->db->where('type', 'about_school');
            $this->db->update('bot_profile', $data);

            $data['description'] = $this->input->post('management_staff');
            $this->db->where('type', 'management_staff');
            $this->db->update('bot_profile', $data);

            $data['description'] = $this->input->post('admission_procedure');
            $this->db->where('type', 'admission_procedure');
            $this->db->update('bot_profile', $data);

            $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
            redirect(base_url() . 'index.php?admin/bot_profile/', 'refresh');
        }

        if ($param1 == 'upload_logo') {

            $date = new DateTime();
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/bot_logo.png');
            $cfile = new CURLFile(realpath('uploads/bot_logo.png'),'image/png', $date->getTimestamp().'.png');

            $ch = curl_init($this->config->item('smsapi').'Blob/');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, array("filedata" => $cfile));
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: multipart/form-data'));
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        if ($param1 == 'upload_baner') {
            $date = new DateTime();
            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/bot_baner.png');

            $cfile = new CURLFile(realpath('uploads/bot_baner.png'),'image/png', $date->getTimestamp().'.png');
        ...
        $page_data['page_name']  = 'bot_profile';
        $page_data['page_title'] = get_phrase('bot_profile');
        $this->load->view('backend/index', $page_data);
    }

    // Manage Bot Profile
    function  bot_management($typeId = '', $param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        if ($param1 == 'add') {
            ...
            if($param2 == 'video')
            {
                ...
                $NewFileName = 'uploads/'.$date->getTimestamp().'.'.$ext;
                $res = move_uploaded_file($_FILES['videofile']['tmp_name'],  $NewFileName);
                $cfile = new CURLFile(realpath($NewFileName),$_FILES['videofile']['type'], $date->getTimestamp().'.'.$ext);
            }
            else
            {
                $ext = pathinfo($_FILES['userfile']['name'], PATHINFO_EXTENSION);
                $NewFileName = 'uploads/'.$date->getTimestamp().'.'.$ext;
                $res =  move_uploaded_file($_FILES['userfile']['tmp_name'],  $NewFileName);
                $cfile = new CURLFile(realpath($NewFileName),$_FILES['userfile']['type'], $date->getTimestamp().'.'.$ext);
            }
            ...
            $this->db->insert('bot_responses', $data);
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'index.php?admin/bot_management/'.$typeId, 'refresh');
        }
        ...
        if ($param1 == 'update') {
           ...
           // $data['image']       			= $this->input->post('image');
            ...
            if($_FILES['videofile']['tmp_name'] != null || $_FILES['userfile']['tmp_name'] != null)
            {
                $date = new DateTime();
                $NewFileName = '';
            ...
            $this->db->where('bot_response_id', $param2);
            $this->db->update('bot_responses', $data);
            redirect(base_url() . 'index.php?admin/bot_management/'.$typeId, 'refresh');
        }

        if ($param1 == 'delete') {
            $this->db->where('bot_response_id', $param2);
            $this->db->delete('bot_responses');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/bot_management/'.$typeId, 'refresh');
        }

        $type_name = $this->crud_model->get_response_name($typeId);
        $page_data['page_name']  	= 'bot_management';
        $page_data['page_title'] 	= get_phrase('bot_management'). " - ".get_phrase($type_name);
        $page_data['type_id'] 	= $typeId;
        $page_data['type_name'] 	= $type_name;
        $this->load->view('backend/index', $page_data);
    }

    //Send Message

    function send_message($class_id = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect('login', 'refresh');
        ...
        if($param2 == 'send')
        {
            ...
            $url = $this->config->item('smsbot').'SendMessage?parentId='.$param3.'&message='.urlencode($this->input->post('message'));
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('Message Sent Successfully.'));
            redirect(base_url() . 'index.php?admin/send_message/'.$class_id, 'refresh');
        }

        $page_data['page_name']  	= 'send_message';
        $page_data['page_title'] 	= get_phrase('send_message_to_parent'). " - ".get_phrase('class')." : ".
            $this->crud_model->get_class_name($class_id);
        $page_data['class_id'] 	= $class_id;
        $this->load->view('backend/index', $page_data);
    }
